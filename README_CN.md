...
<a id="prompt-cn-s01-x01"></a>
#### 图例：像素对战重制（X）

```text
将经典掌机怪物对战截图重制为高细节像素艺术。保留怀旧的俯视 JRPG 布局，同时提升材质层次、角色可读性与特效完成度。画面中使用两只随机关都地区怪物对战，UI 现代极简，血条清晰，技能特效有冲击力但仍保持 16-bit 时代观感。
```

Source (X): https://x.com/i/web/status/2027157212863631567

<a id="prompt-cn-s01-x02"></a>
#### 图例：透明掌机取景框（X）

```text
生成一张写实特写：双手握着一台现代透明黑色掌机（复古设备灵感），上传的参考图准确显示在屏幕区域。强调真实反射、塑料折射、轻微指纹与柔和桌面环境光，让设备质感可信。
```

Source (X): https://x.com/i/web/status/2027365890514194633

<a id="prompt-cn-s01-x03"></a>
#### 图例：新地区跟随镜头（X）

```text
在与参考图相同世界观和机位透视下，生成全新地图区域并切换天气（例如雾海小镇、雨林或雪岭）。主角身后跟随一只关都地区伙伴怪物。保持像素网格一致性、地块逻辑与探索氛围。
```

Source (X): https://x.com/i/web/status/2027401926024048749

<a id="prompt-cn-s01-x04"></a>
#### 图例：世界地图续帧截图（X）

```text
生成该游戏区域的“下一帧截图”，仿佛玩家向前走了几步。保持地图风格、透视关系与整体气质一致；主角后方保留一只跟随怪物；加入细微环境叙事道具，让场景更有生命力。
```
...
<a id="prompt-cn-s06-x01"></a>
#### 图例：工笔汉服吹风机场景（X）

```text
一幅传统中国工笔重彩画，纸张为陈旧宣纸肌理。画面中，一位身着盛唐华服的贵妇端坐木凳，手持现代吹风机吹拂长发。要求线描精准、矿物色层次清晰、衣褶优雅，并以留白平衡构图；在不破坏古典审美的前提下自然融入现代器物。
```

Source (X): https://x.com/i/web/status/1992366309288595681

<a id="prompt-cn-s06-x02"></a>
#### 图例：唐风仙子与扫地机器人（X）

```text
创作一幅工笔风格中国水墨重彩画，底材为陈旧纹理宣纸。唐风仙子身着红、米、青绿飘带长裙，发髻簪牡丹，立于黑色圆形扫地机器人之上，仿佛悬浮法器。强调身姿轻盈、笔触细腻，以及古典服饰与现代家电的诗性反差。
```

Source (X): https://x.com/i/web/status/1992450418291466517

<a id="prompt-cn-s06-x03"></a>
#### 图例：清明长卷复现（X）

```text
生成一幅长卷式中国古画场景，模拟宋代叙事绘画：散点透视、繁密市井故事与精细建筑结构，风格参考《清明上河图》。使用丝绢陈化纹理、墨彩分层，以及桥梁、市集、河运等连续群像调度，呈现流动叙事。
```

Source (X): https://x.com/i/web/status/1992469131438719122

<a id="prompt-cn-s06-x04"></a>
#### 图例：桃花源设定参考板（X）

```text
设计“桃花源”3D 神话游戏场景概念参考板：隐藏仙谷宫殿、古典中式建筑、真实斗拱结构。画面需包含多视角环境标注、石径与桃林雾气、山体纵深，以及建筑材质、旗幡布料、水体与光照情绪说明。
```
...
<a id="prompt-cn-s07-x01"></a>
#### 图例：周末闪光自拍（X）

```text
超写实竖版手机人像（4:5，8K）。呈现明亮周末室内自拍氛围：皮肤纹理自然、毛孔真实、妆面轻透，神态自信放松。使用正面柔闪与环境日光混合照明，眼神高光清晰，背景轻微虚化，构图符合社媒原生观感。
```

Source (X): https://x.com/i/web/status/2025154229870948710

<a id="prompt-cn-s07-x02"></a>
#### 图例：高保真双人时尚人像（X）

```text
生成高端双人时尚写真：面部相似度高、皮肤细节真实、色彩分离具有电影质感。姿态语言偏编辑风，主光/辅光/轮廓光控制精确，保留轻微镜头质感，整体成片高级但表情自然不过度摆拍。
```
